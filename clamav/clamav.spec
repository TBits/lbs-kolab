...
%if 0%{?with_systemd} < 1 && 0%{?with_upstart} < 1
...
%if 0%{?opensuse_bs} < 1
Requires(post):		user(%{milteruser})
Requires(preun):	user(%{milteruser})
%endif
Requires(pre):		%{_initrddir}
Requires(postun):	%{_initrddir} initscripts
Requires(post):		chkconfig clamav-milter
Requires(preun):	chkconfig initscripts clamav-milter
...
install -d -m 0755 \
	%{buildroot}/%{_sysconfdir}/{mail,clamd.d,logrotate.d,tmpfiles.d} \
	%{buildroot}/%_var/{log,run} \
...
rm -f	%{buildroot}/%{_sysconfdir}/clamd.conf.sample \
...
%else
install -m 0644 -p %SOURCE520		%{buildroot}/%{pkgdatadir}/
smartsubst 's!/usr/share/clamav!%{pkgdatadir}!g' %{buildroot}/%{pkgdatadir}/clamd-wrapper
%endif

install -m 0755 -p %SOURCE100		%{buildroot}/%{pkgdatadir}/
...
install -D -m 0755 -p %SOURCE8		%{buildroot}/%{_sbindir}/clamav-notify-servers
touch %{buildroot}/%freshclamlog

install -D -p -m 0755 %SOURCE200	%{buildroot}/%{pkgdatadir}/freshclam-sleep
...
sed -e 's!<SERVICE>!scan!g;s!<USER>!%scanuser!g' \
    etc/clamd.conf.sample > %{buildroot}/%{_sysconfdir}/clamd.d/scan.conf

%if 0%{?with_systemd} < 1 && 0%{?with_upstart} < 1
sed -e 's!<SERVICE>!scan!g;' %{buildroot}/%{pkgdatadir}/template/clamd.init \
    > %{buildroot}/%{_initrddir}/clamd.scan
%endif

%if 0%{?with_upstart}
install -D -p -m 0644 %SOURCE410 %{buildroot}/%{_sysconfdir}/init/clamd.scan.conf
%endif
...
cat << EOF > %{buildroot}/%{_sysconfdir}/tmpfiles.d/clamd.scan.conf
d %scanstatedir 0710 %scanuser %scanuser
EOF

touch %{buildroot}/%scanstatedir/clamd.{sock,pid}
...
sed -r \
    ...
    etc/clamav-milter.conf.sample > %{buildroot}/%{_sysconfdir}/mail/clamav-milter.conf

%if 0%{?with_systemd} < 1 && 0%{?with_upstart} < 1
install -D -p -m 0755 %SOURCE320 %{buildroot}/%{_initrddir}/clamav-milter
%endif

%if 0%{?with_upstart}
install -D -p -m 0644 %SOURCE310 %{buildroot}/%{_sysconfdir}/init/clamav-milter.conf
%endif
...
cat << EOF > %{buildroot}/%{_sysconfdir}/tmpfiles.d/clamav-milter.conf
d %milterstatedir 0710 %{milteruser} %{milteruser}
EOF

rm -f %{buildroot}/%{_sysconfdir}/clamav-milter.conf.sample
touch %{buildroot}/{%milterstatedir/clamav-milter.{socket,pid},%milterlog}

%{!?with_upstart:  rm -rf %{buildroot}/%{_sysconfdir}/init}
%{!?with_systemd:  rm -rf %{buildroot}/%{_unitdir}}
%{!?with_sysv:     rm -f  %{buildroot}/%{_initrddir}/*}
%{!?with_sysv:     rm -rf %{buildroot}/%_var/run/*/*.pid}
%{!?with_tmpfiles: rm -rf %{buildroot}/%{_sysconfdir}/tmpfiles.d}

%if 0%{?with_systemd} < 1
# keep clamd-wrapper in every case because it might be needed by other
# packages
ln -s %{pkgdatadir}/clamd-wrapper		%{buildroot}/%{_initrddir}/clamd-wrapper
%endif
