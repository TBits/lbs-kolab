...
%{?!el6:sed -i -e 's/^AC_PROG_LIBTOOL/LT_INIT/g' configure.ac}
...
%configure --disable-dependency-tracking --disable-static --without-libevent --with-boost=/usr %{java_configure} %{ruby_configure} %{erlang_configure} %{golang_configure} %{php_configure} --docdir=%{?_pkgdocdir}%{!?_pkgdocdir:%{_docdir}/%{name}-%{version}}
...
(
  ...
  sh bootstrap.sh
  %configure --disable-static --without-php --libdir=%{_libdir}
  make %{?_smp_mflags}
  (
      cd java
      ant dist
